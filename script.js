...
const select = document.querySelectorAll(".select-placeholder");
...
// au chargement (important pour tes query params 👀)
updateSelectColor();
...
select.forEach(e => {
  e.addEventListener("change", updateSelectColor);  
})
...
function displayValue(val) {
  label.innerText = val == 500 ? "500+" : val
...
range.addEventListener("input", () => {
  input.value = displayValue(range.value);
